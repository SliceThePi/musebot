...
const characters = new (require("./lib/characters.js"))("./characters.json");
...
function reply(){
  ...
  if(arguments.length == 2 && !arguments[1].context){
    ...
    if(arguments[1].context.commands)
      name = translate(arguments[0], "commands:running", context);
    else
      name = translate(arguments[0], "commands:running-unknown", context);
  } else if(arguments.length == 3){
    message = translate(arguments[0], message, arguments[2]);
    context = arguments[2];
...
let parameters = {
  ...
  /**
   * A valid character name, which will be added to the
   * context.input.characternames array.
   */
  "charactername": (context) => {
    if(!context.text){
      context.errorkey = "commands:error.missing-parameter";
      context.error = "charactername";
      return false;
    }
    let match = context.text.match(/^"([^"]+)"(?:[ \n]([^]*))?$/);
    if(!match){
      context.errorkey = "commands:error.invalid.charactername";
    ...
    if(!context.input.characternames)
      context.input.characternames = [];
    context.input.characternames.push(match[1]);
    context.text = match[2];
    return true;
  },
  /**
...
client.on("message", (message) => {
  if(message.content.startsWith(config.prefix)){
    console.log(formatDate(Date.now()) + JSON.stringify(message.author.tag) + ": " + JSON.stringify(message.content));
  	try{
